...
func TestNewRecord(t *testing.T) {
	...
	// IDが負であることを確認（DBで自動生成される）
	if record.ID >= 0 {
		t.Errorf("Expected ID to be negative (auto-generated by DB), got %d", record.ID)
	}
	...
	if record.ProjectID != projectID {
		t.Errorf("Expected ProjectID to be %d, got %d", projectID, record.ProjectID)
	}
