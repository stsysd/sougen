...
func TestNewProject(t *testing.T) {
	...
	// IDフィールドが負であることを確認（DBで自動生成される）
	if project.ID >= 0 {
		t.Errorf("Expected ID to be negative (auto-generated by DB), got %d", project.ID)
	}
...
func TestLoadProject(t *testing.T) {
	...
	if project.ID != id {
		t.Errorf("Expected ID %d, got %d", id, project.ID)
	}
...
func TestLoadProjectWithEmptyName(t *testing.T) {
	_, err := LoadProject(int64(1), "", "description", testTime(), testTime())
	if err == nil {
...
func TestLoadProjectWithZeroCreatedAt(t *testing.T) {
	_, err := LoadProject(int64(1), "name", "description", testZeroTime, testTime())
	if err == nil {
...
func TestLoadProjectWithZeroUpdatedAt(t *testing.T) {
	_, err := LoadProject(int64(1), "name", "description", testTime(), testZeroTime)
	if err == nil {
...
func TestProjectValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Valid project with ID<0 (new project)",
			project: &Project{
				ID:          -1,
				Name:        "project",
			...
			expectError: false,
			description: "ID<0の場合も検証をパスすること（新規プロジェクト）",
		},
		{
			name: "Empty name",
			project: &Project{
